...
fn main() {
	...
	loop {
		...
		if last_length != test_content.chars().count() as u32 {
			println!("Length {} in {} seconds, {} iterations checked.", last_length.to_formatted_string(&Locale::en), now.elapsed().as_secs().to_formatted_string(&Locale::en), iterations.to_formatted_string(&Locale::en));
		...
		if sha.result_str() == target_hash {
			...
			println!("FOUND SHA256 COLLISION IN {} SECONDS, {} ITERATIONS!", now.elapsed().as_secs().to_formatted_string(&Locale::en), iterations.to_formatted_string(&Locale::en));
